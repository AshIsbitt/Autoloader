...
import pyinputplus as pyplus  # type: ignore
...
def runAutoLoad():
    ...
    for value in jsonData["app"]:
        if str(platform.platform())[0:5] == "macOS":
            ...
            print(
                "Please submit the following message on the github repo: https://github.com/AshIsbitt/Autoloader/issues"
            )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    subparser.add_parser("run", help="Execute all saved locations in Autoloader")
    subparser.add_parser("view", help="Display locations in config file")
    subparser.add_parser("erase", help="Erase config file")
    ...
    addParser.add_argument(
        ...
        type=str,
        required=True,
        help="Type of location to save. Options are 'web', 'app' 'dir'",
    )
    addParser.add_argument(
        "-l",
        "--location",
        type=str,
        required=True,
        help="Location ie website or file location to add",
    )

    remParser.add_argument(
        "-c",
        "--category",
        type=str,
        required=True,
        help="Type of location to remove. Options are 'web', 'app' 'dir'",
    )
    remParser.add_argument(
        "-l",
        "--location",
        type=str,
        required=True,
        help="Location ie website or file location to remove",
    )
